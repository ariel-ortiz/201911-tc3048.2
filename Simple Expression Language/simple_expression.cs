...
public class CILVisitor {
    ...
    public String Visit(Prog node) {
        return
            "// CIL code generated by the Simple Expression compiler.\n"
            + "\n"
            + ".assembly 'example' { }\n"
            + ".assembly extern 'wyvernlib' { }\n"
            + "\n"
            + ".class public 'Simple' extends['mscorlib']'System'.'Object' {\n"
            + "\t.method public static void 'start'() {\n"
            + "\t\t.entrypoint\n"
            + Visit((dynamic) node[0])
            + "\t\tcall int32 class ['wyvernlib']'Wyvern'.'Utils'::'Printi'(int32)\n"
            + "\t\tpop\n"
            + "\t\tret\n"
            + "\t}\n"
            + "}\n";
...
public class Driver {
    public static void Main() {
        ...
        try {
            var ast = parser.Prog();
            var result = new CILVisitor().Visit((dynamic) ast);
